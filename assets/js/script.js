...
const gameOver = gameWon => {
    for (let index of winCombos[gameWon.index]) {
        ...
        document.getElementById(index).style.backgroundColor = '#abceeb';
    }
    ...
    declareWinner(gameWon.player === user ? "Você ganhou o Jogo!" : "Peterson ganhou o Jogo!");
    }
...
const checkTie = () => {
    ...
    if (emptySquares().length === 0) {
        ...
        for (let cell of cells) {
            cell.style.backgroundColor = "#3f87c2"
            cell.removeEventListener('click', handleClick, false);
        }

        declareWinner("O jogo é empete!");
        return true;
    }
